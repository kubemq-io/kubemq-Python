...
class TlsConfig(BaseModel):
    ...
    enabled: bool = Field(default=False, description="Indicates whether TLS is enabled or not")
    ...
    ca_file: str = Field(default="", description="Path to the TLS CA (Certificate Authority) file")

    @field_validator("cert_file", "key_file", "ca_file")
    def validate_file_path(cls, v: str) -> str:
        """Validate that the file exists if a path is provided."""
    ...
    def validate_enabled_config(self) -> "TlsConfig":
        """Validate that cert and key files are specified when TLS is enabled."""
        if self.enabled:
