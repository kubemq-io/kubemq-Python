...
class QueueMessage(BaseModel):
    ...
    # Instance attributes
    id: str | None = Field(default=None, description="The unique identifier for the message")
    ...
    expiration_in_seconds: int = Field(
        default=0, ge=0, description="Time in seconds after which the message expires"
    )
    ...
    def validate_expiration(cls, v: int) -> int:
        ...
        if v > cls.MAX_EXPIRATION_SECONDS:
            raise ValueError(
                f"Expiration cannot exceed {cls.MAX_EXPIRATION_SECONDS} seconds (12 hours)"
            )
        return v
    ...
    def decode(cls, pb_message: pbQueueMessage) -> QueueMessage:
        ...
        return cls(
            ...
            attempts_before_dead_letter_queue=pb_message.Policy.MaxReceiveCount
            if pb_message.Policy
            else 0,
