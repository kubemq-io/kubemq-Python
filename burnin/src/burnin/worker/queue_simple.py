"""Queue Simple worker: send_queue_message + receive_queue_messages with auto_ack.
...
class QueueSimpleWorker(BaseWorker):
    """Queue Simple pattern: send + receive with auto_ack (no manual ack)."""
    ...
    async def start_consumers_only(self) -> None:
        for i in range(self._pattern_config.consumers_per_channel):
            ...
            self._start_task(f"queue-simple-consumer-{self.channel_index:04d}-{i}", self._run_consumer(consumer_id))
    ...
    async def start_producers_only(self) -> None:
        for i in range(self._pattern_config.producers_per_channel):
            ...
            self._start_task(f"queue-simple-producer-{self.channel_index:04d}-{i}", self._run_producer(producer_id))

    async def _run_consumer(self, consumer_id: str) -> None:
        """Consumer task: poll for messages with auto_ack=True."""
        mc.set_active_connections(PATTERN, 1)
        logger.info("queue_simple consumer %s started on %s", consumer_id, self.channel_name)
            ...
            try:
                ...
                if response.is_error:
                    if "timeout" not in str(response.error).lower():
                        logger.debug("queue_simple receive error: %s", response.error)
                ...
                if not self._consumer_stop.is_set():
                    logger.debug("queue_simple poll error: %s", e)
                    self.record_error("receive_failure")
                    await asyncio.sleep(1.0)

        mc.set_active_connections(PATTERN, 0)
        logger.info("queue_simple consumer %s stopped", consumer_id)

    async def _run_producer(self, producer_id: str) -> None:
        """Producer task: send queue messages."""
        seq = 0
        logger.info("queue_simple producer %s started on %s", producer_id, self.channel_name)
            ...
            try:
                if self._benchmark:
                    ...
                    if not result.is_error:
                        ...
                        logger.debug("queue_simple send error: %s", result.error)
                ...
                logger.debug("queue_simple send error: %s", e)
                self.record_error("send_failure")

        logger.info("queue_simple producer %s stopped", producer_id)
