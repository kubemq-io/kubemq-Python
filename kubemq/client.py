...
class _QueueUpstreamSender:
    ...
    def _send(self, message: QueueMessage) -> [QueueSendResult, None]:
        if not self._allow_new_messages:
            raise ConnectionError("Client is not connected to the server and cannot send messages.")
        response_result = threading.Event()
        response_container = {}
        message_id = message.id
        queue_upstream_request = message.encode(self._connection.client_id)
        with self._lock:
            self._response_tracking[queue_upstream_request.RequestID] = (
                response_container, response_result, message_id)
        self._sending_queue.put(queue_upstream_request)
        response_result.wait()
        response: QueuesUpstreamResponse = response_container.get('response')
        with self._lock:
            if response is not None:
                del self._response_tracking[queue_upstream_request.RequestID]
    ...
    def _handle_disconnection(self):
        with self._lock:
            ...
            # Set error on all response containers
            for request_id, (response_container, response_result, message_id) in self._response_tracking.items():
    ...
    def _send_queue_stream(self):
        ...
        while not self._shutdown_event.is_set():
            try:
                with self._lock:
                    self._allow_new_messages = True
                responses = self._clientStub.QueuesUpstream(send_requests())
                for response in responses:
                    if self._shutdown_event.is_set():
                        break
                    ...
                    with self._lock:
                        if response_request_id in self._response_tracking:
                            response_container, response_result, message_od = self._response_tracking[
                                response_request_id]
                            response_container['response'] = response
                            response_result.set()
            except grpc.RpcError as e:
